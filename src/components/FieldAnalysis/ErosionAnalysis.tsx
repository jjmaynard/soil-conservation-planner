    ...
    try {
      ...
      if (geeData?.geeAssessment) {
        ...
        setErosionData({
          ...
          meanTValue: (stats as any).mean_t_value, // Actual T value from SSURGO (optional)
          areas: [
            {
              risk: 'Low',
              acres: 0,
          ...
          factors: {
            slope: `${(stats as any).mean_slope_pct?.toFixed(1) || 0}%`,
            kFactor: (stats as any).mean_k_factor?.toFixed(3) || '0.000',
            spi: (stats as any).mean_spi?.toFixed(1) || '0.0',
            runoffFactor: (stats as any).mean_runoff_factor?.toFixed(2) || '0.00',
          },
          ...
          visualization: {
            ...
            slope: (geeErosion.visualization as any).slope_tile_url,
            kFactor: (geeErosion.visualization as any).k_factor_tile_url,
            spi: (geeErosion.visualization as any).spi_tile_url,
            runoff: (geeErosion.visualization as any).runoff_tile_url,
            tValue: (geeErosion.visualization as any).t_value_tile_url,
        ...
        if (storedData) {
          ...
          if (parsed.geeAssessment) {
            ...
            setErosionData({
              ...
              riskLevel: combined.combined_risk,
              isIndex: true,
              meanTValue: (stats as any).mean_t_value,
              areas: [
                {
                  risk: 'Low',
                  acres: 0,
