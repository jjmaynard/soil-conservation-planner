...
export type CropType = 'annual' | 'perennial' | 'permanent' | 'pasture' | 'forest' | 'developed' | 'water' | 'other'
...
export function validateTransition(
  ...
  if (toType === 'permanent') {
    return `${toName} typically requires 3-7 years to establish. Single-year detection may indicate misclassification${confidence ? ` (${confidence}% confidence)` : ''}.`
  }
  
  // Transitioning FROM permanent to annual is very suspicious
  if (fromType === 'permanent' && (toType === 'annual' || toType === 'pasture')) {
    return `Unlikely transition from established ${fromName} to ${toName}. Permanent crops are not typically removed after establishment${confidence ? ` (${confidence}% confidence)` : ''}.`
  }
  
  // Forest/developed appearing briefly is suspicious
  if ((toType === 'forest' || toType === 'developed') && fromType !== toType) {
    return `Land use change to ${toName} is typically permanent. Brief detection may indicate misclassification${confidence ? ` (${confidence}% confidence)` : ''}.`
  }
 ...
 */
export function analyzeCropHistory(history: Array<{ year: number; cropType?: CropType; cropName: string; confidence?: number }>) {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (current.cropType === 'permanent') {
      ...
      if (prevDifferent && nextDifferent) {
        warnings.push({
          year: current.year,
          warning: `Single year of ${current.cropName} is highly unlikely. Permanent crops take years to establish and produce${current.confidence ? ` (${current.confidence}% confidence)` : ''}.`
        })
      }
    }
