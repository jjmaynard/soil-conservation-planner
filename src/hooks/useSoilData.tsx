...
export function useSoilData() {
  ...
  const fetchSoilProfile = useCallback(
    async (lat: number, lon: number, depth?: SoilDepth): Promise<SoilProfile> => {
      ...
      try {
        const profile = await SoilAPI.predictSoilProperties(
          lat,
          lon,
          depth || '0-5cm',
          true
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to fetch soil data'
  ...
  const fetchMIRData = useCallback(
    async (lat: number, lon: number): Promise<MIRAnalysisResponse | null> => {
      try {
  ...
  const fetchMultipleDepths = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to fetch soil profiles'
        setError(errorMessage)
        throw err
      } finally {
        setLoading(false)
