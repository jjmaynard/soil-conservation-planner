...
function wgs84ToAlbers(lon: number, lat: number): { x: number; y: number } {
  ...
  const q = (1 - e * e) * (Math.sin(phi) / (1 - e * e * Math.sin(phi) * Math.sin(phi)) - 
...
export default async function handler(
  ...
  try {
    ...
    console.log(`[CDL Value Proxy] Transforming WGS84 (${latNum}, ${lonNum}) to Albers (${albers.x.toFixed(2)}, ${albers.y.toFixed(2)})`)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorText.includes('Failed to get value') || errorText.includes('No data')) {
          console.log(`[CDL Value Proxy] No CDL data available for year ${year} at this location. Returning 0.`)
